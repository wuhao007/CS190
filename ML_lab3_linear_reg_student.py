...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda (label, prediction): squaredError(label, prediction)).sum() / labelsAndPreds.count()) ** 0.5
...
def twoWayInteractions(lp):
    ...
    """
    two_way = DenseVector(map(lambda (x, y): x * y, itertools.product(lp.features, lp.features)))
    features = np.hstack((lp.features, two_way))
