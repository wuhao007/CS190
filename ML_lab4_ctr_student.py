...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    key = sorted(map(lambda x: OHEDict[x], rawFeats))
    return SparseVector(numOHEFeats, key, [1.] * len(key))
...
rawTrainData, rawValidationData, rawTestData = rawData.randomSplit([.8, .1, .1], seed)
# Cache the data
rawTrainData.cache()
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    """   
    s = point.split(',')
    return LabeledPoint(s[0], oneHotEncoding(parsePoint(point), OHEDict, numOHEFeats))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    key = sorted([OHEDict[x] for x in rawFeats if x in OHEDict])
    return SparseVector(numOHEFeats, key, [1.] * len(key))
...
trainingPredictions = OHETrainData.map(lambda x: getP(x.features, sortedWeights, model0.intercept))
...
def evaluateResults(model, data):
    ...
    """
    print data.map(lambda x: x.label - model.weights.dot(x.features))
